...
final class MainViewController: UITabBarController {
    ...
    private lazy var chats: ConversationListController = {
        let vc = chat_uikit.ComponentsRegister.shared.ConversationsController.init()
        vc.tabBarItem.tag = 0
    ...
    private lazy var contacts: ContactViewController = {
        let vc = chat_uikit.ComponentsRegister.shared.ContactsController.init(headerStyle: .contact)
        vc.tabBarItem.tag = 1
    ...
    private func setupDataProvider() {
        //`userProfileProvider` is the provider of user data. It can't exist at the same time as userProfileProviderOC. userProfileProviderOC is implemented using closures.
        ChatUIKitContext.shared?.userProfileProvider = self
        ChatUIKitContext.shared?.userProfileProviderOC = nil
        //`groupProvider` usage same as above
...
extension MainViewController: ThemeSwitchProtocol {
    ...
    func switchTheme(style: chat_uikit.ThemeStyle) {
