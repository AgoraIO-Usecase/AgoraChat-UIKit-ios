...
- (void)setUserId:(NSString *)userId {
    _userId = userId;
    _deleteButton.hidden = ![userId isEqualToString:AgoraChatClient.sharedClient.currentUsername];
    __weak typeof(self)weakSelf = self;
    id<EaseChatKitManagerGeneralDelegate> delegate = EaseChatKitManager.shared.generalDelegate;
    ...
    if (delegate && [delegate respondsToSelector:@selector(getUserInfo:result:)]) {
        [delegate getUserInfo:userId result:^(AgoraChatUserInfo * _Nonnull userInfo) {
