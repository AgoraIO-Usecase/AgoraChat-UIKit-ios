                               ...
                               imgSize:(CGSize)aSize
{
    ...
    void (^block)(CGSize aSize) = ^(CGSize aSize) {
        ...
        if (!weakself.model.thread) {
            if (weakself.model.message.msgOverView) {
                space = 12*2;
            }
        }
        if (!weakself.model.thread) {
            if (weakself.model.message.msgOverView) {
                [weakself Ease_updateConstraints:^(EaseConstraintMaker *make) {
                    ...
                    make.height.Ease_equalTo(KEMThreadBubbleWidth*0.4+layoutSize.height+4+12*2);
                }];
                ...
                [weakself.threadBubble Ease_makeConstraints:^(EaseConstraintMaker *make) {
                    make.top.equalTo(weakself.photo.ease_bottom).offset(4);
                    make.left.equalTo(weakself).offset(12);
...
- (void)setModel:(EaseMessageModel *)model
{
    ...
    if (model.message.msgOverView.threadId.length > 0 && model.thread == nil) {
    ...
    self.photo.hidden = !model.message.msgOverView;
