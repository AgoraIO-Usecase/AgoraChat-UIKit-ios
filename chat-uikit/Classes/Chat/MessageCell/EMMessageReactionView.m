...
- (void)layoutSubviews {
    ...
    CGFloat x = _direction == AgoraChatMessageDirectionSend ? self.bounds.size.width - w : 0;
...
- (void)setReactionList:(NSArray<AgoraChatMessageReaction *> *)reactionList {
    ...
    self.hidden = NO;
    uint64_t allCount = 0;
    for (AgoraChatMessageReaction *reaction in _reactionList) {
...
- (NSArray<UIBezierPath *> *)hollowedOutPathsInView:(UIView *)view {
    if (!self.hidden) {
        CGRect rect = [_contentView.superview convertRect:_contentView.frame toView:view];
        return @[[UIBezierPath bezierPathWithRoundedRect:rect cornerRadius:rect.size.height / 2]];
    }
    return nil;
}
